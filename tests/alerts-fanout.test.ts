 ...
 * pasaba a reintentar cada minuto en vez de cada 3, aunque alguien SÍ
 * hubiera recibido la alerta.
 *
 * El fix: mirar el último envío CON ÉXITO y el último FALLIDO por
 * separado -- si cualquiera tuvo éxito hace poco, se calla los 3 min
 * completos, sin importar si hay un fallo más reciente todavía.
 *
 * (Historial del mismo día: 2026-08-17 pasó de 10 min/2 min original, a 0
 * a petición del usuario, a 3 min/1 min tras ver que 0 mandaba demasiados
 * correos en un ataque sostenido -- ver lib/alerts.ts.)
 *
...
prueba(
  ...
  async (deviceId) => {
    ...
    assert.equal(
      despues.length,
      2,
      "el 'sent' de hace 30s debe callar el canal 3 min completos, sin importar que el 'failed' sea más reciente",
    );
  },
);
...
prueba("sin ningún 'sent' reciente, un 'failed' reciente sigue frenando (ventana corta)", async (deviceId) => {
  ...
  assert.equal(despues.length, 1, "un fallo de hace 5s sigue dentro del cooldown corto (1 min): no debe reintentar");
});
...
prueba("pasada la ventana corta del fallo, sin ningún 'sent' reciente, sí se reintenta", async (deviceId) => {
  ...
  const despues = await filasAlertLog(deviceId, "whatsapp");
  assert.ok(despues.length >= 2, "al minuto de un fallo, sin ningún éxito reciente, debe reintentar");
});
